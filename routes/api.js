...
const {
  ...
  findActivityStaff,
  countActivityStaff,
} = require("../services/staffFunction");
...
router.get("/account/info", async (req, res) => {
  if (req.currentUser) {
    ...
    if (req.query.stk && req.query.bankCode) {
      found = await AccountInfo.getBySTKAndBankCode(
        req.query.stk,
        req.query.bankCode
      );
...
router.post("/account/addMoney", async (req, res) => {
  ...
  if (stk && bankCode) {
    ...
    if (found) {
      const rs = await AccountInfo.addMoneyInternal(
        from,
        stk,
        money,
        message,
      ...
      if (ToInfo) {
        ...
        sendMail(
          ToInfo.email,
          "Nhận tiền",
          `Bạn vừa được nhận tiền thành công:\n
                STK gửi: ${from} \n
                STK nhận: ${stk} \n
        ...
        if (from != "ADMIN") {
          ...
          sendMail(
            ...
            "Chuyển tiền",
            `Bạn vừa chuyển tiền thành công:\n
                    STK gửi: ${from} \n
                    STK nhận: ${stk} \n
...
router.get("/get-all-transfer", async (req, res) => {
  ...
  const found = await Transfer.getActivityByDate(
    stk,
    page,
    limit,
    fromDate,
...
router.post("/change-password", async (req, res) => {
  ...
  if (st) {
    ...
    if (found) {
      if (
        (await User.verifyPassword(password, found.password)) ||
        !found.password
      ) {
...
router.post("/update-info", async (req, res) => {
  ...
  if (await User.checkDUsername(req.currentUser.id, username))
    return res.json(5); // username da ton tai
...
router.post("/transfer-count-activity", async (req, res) => {
  ...
  if (!foundAccount) return res.json(2); // chuaw set mac dinh hoac chua co tai khoan
  const count = await Transfer.countActivity(
    foundAccount.STK,
    fromDate,
    toDate
  );
...
router.post("/checkaccountid", async (req, res) => {
  ...
  if (!accountId || !bankSecretKey || !bankId || !clientId || !secretKey)
    return res.json(2);
  if (
    clientId !== "wibu" ||
    secretKey !== "36dc50f6-65e5-47c5-80ff-f6dbd8cd3dee"
  )
    return res.json(null);
...
router.post("/listen-external", async (req, res) => {
  ...
  if (!found) return res.json(null);
  const message =
    req.body.message || "Đã có 1 người khác ngân hàng chuyển tiền cho bạn";
  // Chuyển tiền thành công
  if (
    await found.addMoneyExternal(
      requestAccountId,
      money,
      message,
      currency,
...
router.post("/create-savings", async (req, res) => {
  ...
  if (!fromSTK || !term || !currencyUnit || !userID || !balance)
    return res.json(null);
  const rs = await SavingAccount.createRequestSavingAccount(
    fromSTK,
    userID,
    balance,
    term,
...
router.post("/client-saving-info", async (req, res) => {
  ...
  return res.json(
    calculatorProfit(savings.term, savings.beginDate, savings.balance)
  );
});
...
router.post("/withdraw", async (req, res) => {
  ...
  const account = await AccountInfo.addMoneyForSTK(
    STK,
    userID,
    money,
    "Rút tiền tiết kiệm"
